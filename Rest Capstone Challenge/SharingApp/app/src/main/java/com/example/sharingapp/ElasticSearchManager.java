...
public class ElasticSearchManager {
    ...
    private static final String INDEX = "98721932"; // TODO: MUST CHANGE THIS to random number string
    ...
    public static class RemoveBidTask() extends AsyncTask<Bid,Void,Boolean> {
        ...
        protected Boolean doInBackground(Bid... params) {
            ...
            try {
                ...
                if(execute.isSucceeded()) {
                    Log.i("ELASTICSEARCH", "BID Item was successfully deleted");
                    success = true;
                } else {
                    Log.e("ELASTICSEARCH", "BID Item delete failed");
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public static class AddBidTask extends AsyncTask<Bid,Void,Boolean> {
        ...
        protected Boolean doInBackground(User... params) {
            ...
            try {
                DocumentResult execute = client.execute(index);
                if(execute.isSucceeded()) {
                    Log.i("ELASTICSEARCH", "Bid item was successfully added");
                    Log.i("ADDED BID", id);
                    success = true;
                } else {
                    Log.e("ELASTICSEARCH", "Bid item failed to be added");
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public static class GetBidListTask extends AsyncTask<Void,Void,ArrayList<Bid>> {
        ...
        protected ArrayList<Bid> doInBackground(Void... params) {
            ...
            try {
                SearchResult execute = client.execute(search);
                if (execute.isSucceeded()) {
                    List<Bid> remote_bids = (ArrayList<Bid>)execute.getSourceAsObjectList(Bid.class);
                    remote_bids.addAll(bids);
